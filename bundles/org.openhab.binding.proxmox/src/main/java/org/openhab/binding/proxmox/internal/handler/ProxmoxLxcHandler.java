...
/**
 * The {@link ProxmoxLxcHandler} is responsible for handling commands and state updates of a Proxmox LXC container.
 *
...
public class ProxmoxLxcHandler extends BaseThingHandler implements ProxmoxStatusChangedListener<ProxmoxLxc> {
    ...
    private void initializeLxc(@Nullable ThingStatus bridgeStatus) {
        ...
        String node = getThing().getProperties().get(PROPERTY_LXC_NODE);
        nodeName = node;
        if (node == null) {
        ...
        String id = getThing().getProperties().get(PROPERTY_LXC_ID);
        lxcId = id;
        if (id == null) {
        ...
        if (bridgeHandler != null) {
            if (bridgeStatus == ThingStatus.ONLINE) {
                bridgeHandler.registerLxcStatusChangeListener(id, this);
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        if (id == null || node == null) {
            logger.debug("The LXC was not initialized properly. Cannot handle command!");
            return;
        }

        ProxmoxLxc lxc = bridgeHandler.getLxcById(id);
        if (lxc == null) {
    ...
    public void dispose() {
        ...
        if (id != null && bridgeHandler != null) {
            bridgeHandler.unregisterLxcStatusChangeListener(id);
            lxcId = null;
            nodeName = null;
        }
    }
    ...
    public void onRemoved() {
        updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.GONE, "@text/offline.lxc-removed");
    }

    @Override
    public void onGone() {
        updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.GONE, "@text/offline.lxc-gone");
    }
}
